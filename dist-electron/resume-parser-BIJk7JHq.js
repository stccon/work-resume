...
const pdfExtractor = require("./pdf-extractor-CfuUvTk6.js");
...
function isHeaderLine(line, degraded = false) {
  ...
  if (pdfExtractor.KNOWN_SECTION_TITLES.has(normalized)) return true;
  if (line.isShortLine && line.isCommonFont === false) return true;
  if (line.isShortLine && typeof line.yGapBefore === "number" && line.yGapBefore >= Y_GAP_HEADER_THRESHOLD) {
    return true;
  }
  return false;
}
function computeMedianGap(lines) {
  const gaps = [];
...
function splitByBlocks(lines, degraded = false, medianGap = 0) {
  const blocks = [];
  let current = [];
  for (const line of lines) {
    const header = isHeaderLine(line, degraded);
    const yGapBreak = degraded && medianGap > 0 && typeof line.yGapBefore === "number" && line.yGapBefore > medianGap * 1.6;
...
function findBlockHeaderLine(block, degraded = false) {
  ...
  if (degraded) {
    ...
    const byKnownTitle = block.find((l) => pdfExtractor.KNOWN_SECTION_TITLES.has(l.text.replace(/\s+/g, "")));
...
function parseExperienceBlock(block, degraded = false) {
...
function parseProjectBlock(block, degraded = false) {
  const text = block.map((l) => l.text).join(" | ");
  const result = {};
  const rangeRe = new RegExp(DATE_RANGE_RE.source, "i");
  const range = rangeRe.exec(text);
  ...
  const techMatch = blockText.match(/(?:技术栈|tech\s*stack|technologies?|技术)\s*[：:]\s*([^\n|]+)/i);
  if (techMatch) result.techStack = techMatch[1].trim();
  const detailLines = block.filter((l) => l !== headerLine && (!result.role || l.text.trim() !== result.role)).map((l) => l.text).join("\n");
  if (detailLines) {
    result.description = detailLines;
  }
  return result;
}
function parseEducationBlock(block, degraded = false) {
  const text = block.map((l) => l.text).join(" | ");
  const result = {};
  const rangeRe = new RegExp(DATE_RANGE_RE.source, "i");
  const range = rangeRe.exec(text);
...
async function parseResumePdf(filePath) {
  const { extractPdfLayout } = await Promise.resolve().then(() => require("./pdf-extractor-CfuUvTk6.js"));
...
exports.computeMedianGap = computeMedianGap;
...
exports.findBlockHeaderLine = findBlockHeaderLine;
...
exports.splitByBlocks = splitByBlocks;
