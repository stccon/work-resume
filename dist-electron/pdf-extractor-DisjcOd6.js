...
const Y_TOLERANCE = 2;
function reconstructLines(items, yTolerance = Y_TOLERANCE) {
  ...
  const flush = () => {
    ...
    const text = current.map((it, i) => (i > 0 && it.x - current[i - 1].x > (it.width || 4) * 1.2 ? " " : "") + it.text).join("").replace(/\s+/g, " ").trim();
    if (!text) return;
    const maxFontSize = current.reduce((m, it) => Math.max(m, it.fontSize), 0);
    const anyBold = current.some((it) => isBoldFont(it.fontName));
  ...
  for (const item of sorted) {
    const sameLine = item.pageIndex === currentPage && Math.abs(item.y - currentY) <= yTolerance;
    if (!sameLine) {
      flush();
      current = [];
      currentY = item.y;
...
function detectColumnBreaks(lines, minGapRatio = 0.04, minCount = 4) {
  if (lines.length < minCount) return [];
  const xs = lines.map((l) => l.x).sort((a, b) => a - b);
  const maxX = Math.max(...xs);
  const minX = Math.min(...xs);
  const span = maxX - minX;
  if (span <= 0) return [];
  const minGap = span * minGapRatio;
  const gaps = [];
  for (let i = 1; i < xs.length; i++) {
    const w = xs[i] - xs[i - 1];
    if (w >= minGap) gaps.push({ x: (xs[i - 1] + xs[i]) / 2, width: w });
  }
  if (gaps.length === 0) return [];
  gaps.sort((a, b) => b.width - a.width);
  const breaks = [];
  for (const g of gaps) {
    if (g.width < minGap) break;
...
function reconstructLinesPerPage(items, yTolerance = Y_TOLERANCE) {
