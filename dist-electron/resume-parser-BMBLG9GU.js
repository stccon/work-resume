...
function parseExperienceBlock(block) {
  ...
  if (headerLine) {
    ...
    if (dashSplit.length >= 2) {
      const cityIdx = dashSplit.findIndex((seg) => /^[\u4e00-\u9fa5]{2,4}$/.test(seg) || /(?:市|省|区|州|县|Remote|remote)/i.test(seg));
      if (cityIdx > 0 && cityIdx < dashSplit.length - 1) {
        result.company = dashSplit.slice(0, cityIdx).join(" - ").trim();
        result.location = dashSplit[cityIdx].trim();
        result.position = dashSplit.slice(cityIdx + 1).join(" - ").trim();
      } else {
      ...
      if (atSplit.length >= 2) {
        ...
        if (next && (next.isBold || next.fontSize >= 12)) {
